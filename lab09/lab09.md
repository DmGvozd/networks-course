...
2. Почему ICMP-пакет не обладает номерами исходного и конечного портов?
   - ICMP работает на сетевом уровне, а не на транспортном (там номера портов есть), и используется для передачи служебных сообщений между устройствами, а не для обмена данных приложениями, которым нужны эти самые порты (для идентификации), поэтому наличие портов в ICMP-пакете избыточно.
3. Рассмотрите один из ping-запросов, отправленных вашим хостом. Каковы ICMP-тип и кодовый
   номер этого пакета? Какие еще поля есть в этом ICMP-пакете? Сколько байт приходится на поля 
   контрольной суммы, порядкового номера и идентификатора?
   - Type: 8 (Echo (ping) request)
   - Code: 0
   - Checksum, Identifier (BE), Identifier (LE), Sequence Number (BE), Sequence Number (LE), Data (48 bytes)
   - Каждое по два байта
...
<img src="images/traceroute/1.png" width=800 />
<img src="images/traceroute/2.png" width=800 />
...
1. Рассмотрите ICMP-пакет с эхо-запросом на вашем скриншоте. Отличается ли он от ICMP-пакетов
   ...
   - Time to Live стал в 64 раза меньше, в остальном, кажется, ничего нового  
   <img src="images/traceroute/3.png" width=800 />
2. Рассмотрите на вашем скриншоте ICMP-пакет с сообщением об ошибке. В нем больше полей,
   ...
   - Новый "Type: 11 (Time-to-live exceeded)", информацию о пропавшем пакете и IP
   <img src="images/traceroute/5.png" width=800 />
   <img src="images/traceroute/6.png" width=800 />
3. Рассмотрите три последних ICMP-пакета, полученных исходным хостом. Чем эти пакеты
   ...
   - Это уже не ошибки, а ICMP Type: 0 (Echo (ping) reply) от конечного хоста. Структура ответа как в задании с ping'ом. Отличие объясняется тем, что TTL наконец дошёл до адресата, поэтому тот отвечает "эхом", а не "Time exceeded".
   <img src="images/traceroute/7.png" width=800 />
   <img src="images/traceroute/8.png" width=800 />
   <img src="images/traceroute/9.png" width=800 />
4. Есть ли такой канал, задержка в котором существенно превышает среднее значение? Можете
   ...
   - Есть, между 5-м и 7-м узлами.
   - Про 5-й можно догадаться, что он в Санкт-Петербурге (spb в названии), второй содержит ip в названии. Посмотрел через iplocation.net, показывает, что 7-й тоже в Петербурге/Дачное (данные расходятся).
   <img src="images/traceroute/1.png" width=800 />
...
#### Демонстрация работы
- mask.py
- Для винды просил адаптацию у гпт, сам на макоси. Протестировать на другой системе возможности нет(
