...
#### Демонстрация работы
<img src="images/task_B/1.png" width=1000 />
...
#### Решение
Время сборки одного пакета составляет: $\dfrac{56 \cdot 8}{ 128 \cdot 1000 } = 3,5 \ мс$

Этот пакет будет передан от хоста А до хоста Б за: $\dfrac{56 \cdot 8 }{ 2^{20} } = 0,448 \ мс$

Тогда с момента создания бита до момента его декодирования пройдет: $3,5 + 0,448 + 5 = 8,948 \ мс$.
...
#### Решение
Так как в буфере содержится в среднем $10$ пакетов, можем сделать вывод, что $N = 10 + 1 = 11$

Средняя задержка ожидания для пакета равна $0,01$ с, а средняя задержка передачи – $10$ мс (так как 100 пакетов передаются за $1000$ мс). Тогда средняя общая задержка $d = 10 + 10 = 20 \ мс = 0,02 \ с$

Исходя из полученных данных и формулы Литтла, средняя скорость поступления пакета в очередь $a = \dfrac{N}{d} = \dfrac{11}{0,02} = 550$ пакетов в секунду.
...
2. Может. Ситуация, когда первый пакет уже отправился по второй линии к клиенту, но еще не успел до него дойти, как раз описывает случай из вопроса. Второй пакет будет ожидать во входном буфере, пока передача первого не завершится.

Чтобы очередь во вторую линию связи была нулевая, необходимо, чтобы вторая линия связи была свободна в тот момент, когда второй пакет закончит свой путь по первой линии. Так как путь по первой линии связи для каждого пакета занимает $\frac{L}{R_S}$ времени, необходимо как минимум $T = \frac{L}{R_C} - \frac{L}{R_S}$ времени. Тогда за то время, что второй пакет будет двигаться по первой линии связи, первый пакет освободит вторую линию связи и очередь станет нулевой.
...
#### Решение
1. $\Delta = \frac{L}{R} = \frac{85 \cdot 10^4}{15 \cdot 10^6} ≈ 0,0567 \ с$

2. $d = 3 + \frac{0,0567}{1 - 16 \cdot 0,0567} ≈ 3,611 \ с$

3. $d = 0,4 \cdot (3 + \frac{0,0567}{1 - 16 \cdot 0,4 \cdot 0,0567}) + 0,6 \cdot 0 ≈ 1,236 \ с$