...
3. Сколько байт в IP-заголовке? Сколько байт приходится на полезную нагрузку IP-дейтаграммы?
   ...
   - Total Length - Header Length = 56 - 20 = 36 bytes
...
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   1. Какие поля IP-дейтаграммы всегда изменяются от одной дейтаграммы к
      ...
      - Identification, Header Checksum и, наверное, должен еще Time to Live, но у меня он увеличивается на 1 через каждые три пакета (см. скриншоты ниже)
      <img src="images/wireshark/6.png" width=800 />
      <img src="images/wireshark/7.png" width=800 />
      <img src="images/wireshark/8.png" width=800 />
      <img src="images/wireshark/9.png" width=800 />
      <img src="images/wireshark/10.png" width=800 />
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      - Source Address, Destination Address, Total Length, Version, Header Length, Protocol
      - Source Address, Destination Address, Version, Header Length, Protocol. Total Length, думаю, может, но не обязана изменяться. Все будет зависеть от доп. опций, таких, например, как Packet Size. В таком случае возможна фрагментация и изменение Total Length, как я понимаю.
   ...
   3. Как изменяется значение поля Identification IP-дейтаграммы с каждым
      ...
      - Увеличивается на 1
      - Есть, с каждым последующим эхо-запросом значение поля Identification увеличивается на 1 
5. Какое значение содержится в поле Identification (Идентификация), а какое – в поле TTL
   (выберите какой-либо один пакет ICMP)?
   - Identification: 0xf3e6 (62438)
   - Time to Live: 7
   <img src="images/wireshark/11.png" width=800 />
6. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - Да, но это значеня не из моего пакета (который я выбрал в предыдущем пункте) 
7. Найдите серии откликов ICMP, в которых содержатся сообщения об истечении
   ...
   - Time to Live: 251
   <img src="images/wireshark/12.png" width=800 />
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   <img src="images/wireshark/13.png" width=800 />
   1. Было ли это сообщение фрагментировано между двумя или более IP-дейтаграммами? 
      Если да, то сколько фрагментов было создано?
      - Если правильно понимаю, то 3 фрагмента
      <img src="images/wireshark/14.png" width=800 />
   2. Какие поля IP-заголовка изменяются в разных фрагментах?
      - Fragment Offset, Total Length, Header Checksum, Flags
      <img src="images/wireshark/14.png" width=800 />
      <img src="images/wireshark/15.png" width=800 />
      <img src="images/wireshark/16.png" width=800 />
...
#### Демонстрация работы
- Я долго пытался поймать ошибку на разных ресурсах, но мне так и не удалось это осуществить, поэтому блок с обработкой сделан с верой в лучшее... Тем не менее, при помощи Network Link Conditioner получилось имитировать 100% потерю пакетов и проверить работоспособность части обработок.
<img src="images/code/2.png" width=800 />
