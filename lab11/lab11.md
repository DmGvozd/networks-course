...
#### Демонстрация работы
- Оба пункта выполнены в одном файле
- Запуск из папки ```/src```
```
sudo python3 traceroute.py <hostname> [packets_per_hop] [max_hops]
```
...
#### Демонстрация работы
- Запуск из папки ```/src``` по очереди (не совсем понял часть "На первом компьютере"; На лекции говорили, вроде бы, что стоит все это запускать на localhost, т.е. на одном единственном компьютере. Так я и сделал):
```
python3 paint_server.py
```
```
python3 paint_client.py
```
