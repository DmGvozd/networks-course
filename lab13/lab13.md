...
Так как на macos данные команды не работают, все манипуляции я проводил согласно инструкции, которую нашел [на этом сайте](https://support.ucsd.edu/services?id=kb_article_view&sysparm_article=KB0033296).
...
3. Каковы значения Transaction-ID в наборе (Request/ACK) DHCP-сообщений? Опишите
   ...
   - 0x59700671
   - Для сопоставления запроса и ответа между клиентом и сервером
...
6. Объясните назначение срока аренды IP-адреса. Какова длительность срока аренды в
   вашем эксперименте?
   - Указывает, в течение какого времени клиент может использовать выданный IP-адрес. По истечении срока необходим запрос на продление.
   - 1 day (86400)
...
#### Демонстрация работы
- Реализацию заданий объединил в один код
- Использую только встроенные библиотеки python
- Удалось найти все устройства, находящиеся в сети (проверял через настройки роутера)
- запускал как и всегда из папки src, при помощи
      
      sudo python3 scanner.py
- скриншот демонстрации работы программы ниже, в задании Б
