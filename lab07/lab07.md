...
2. Каков IP-адрес у сервера gaia.cs.umass.edu? Каковы номера портов для отправки и приема
   ...
   - IP: 128.119.245.12
   - Port: 80 (и для приёма, и для отправки)
...
3. Какой порядковый номер у SYN TCP-сегмента, который используется для установления
   ...
   - Sequence Number (raw): 3979945825
   - По флагу SYN, в поле Info в квадратных скобках.
...
5. Какой порядковый номер у TCP-сегмента, содержащего команду POST протокола HTTP?
   ...
   - Sequence Number (raw): 3979945826
...
6. Рассмотрите TCP-сегмент, содержащий команду POST протокола HTTP, как первый TCP-сегмент 
   ...
   (включая сегмент, содержащий команду POST протокола HTTP): 1, 622, 1860, 3098, 4336, 5574
   - Были отправлены через: 3.932115, 3.933355, 3.933358, 3.933360, 3,933364 и 3.933365 секунд соответственно.
   - Если я правильно понял, то в моем случае браузер получил ответ от ресурса о доставке 6 сегментов всего лишь в двух ACK-пакетах. Первый (для 1-го и 2-го сегментов) – через 4.072059, второй (для 3-го – 6-го сегментов) – через 4.073212.
   - Значит:

      $RTT_1 = 4.072059 - 3.932115 = 0.139944$
   
      $RTT_2 = 4.072059 - 3.933355 = 0.138704$

      $RTT_3 = 4.073212 - 3.933358 = 0.139854$

      $RTT_4 = 4.073212 - 3.933360 = 0.139852$

      $RTT_5 = 4.073212 - 3,933364 = 0.139848$

      $RTT_6 = 4.073212 - 3.933365 = 0.139847$
...
7. Чему равна пропускная способность (количество байтов, передаваемых в единицу
   ...
   - 1804 kbps = 225500 bytes/s ≈ 225 kB/s (Bits/s A -> B)
   - 49 kbps = 6125 bytes/s ≈ 6 kB/s (Bits/s B -> A)
   - Здесь адреса A и B – мой IP и IP сервера соответственно.
   - Я открыл вкладку Statistics в Wireshark и выбрал Conversations, после чего перешел в раздел TCP. Там, как я понимаю, находится результат аналитики соединения, в том числе количество байтов, передаваемых за единицу времени.
...
#### Демонстрация работы
<img src="images/codetasks/1.png" width=800 />
<img src="images/codetasks/2.png" width=800 />
...
#### Демонстрация работы
<img src="images/codetasks/3.png" width=800 />
...
#### Демонстрация работы
<img src="images/codetasks/5.png" width=800 />
<img src="images/codetasks/6.png" width=800 />
<img src="images/codetasks/7.png" width=800 />
